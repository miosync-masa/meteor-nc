...
def demo_basic_flow():
    ...
    if is_valid:
        ...
        user_info = server.get_user_info(token)
        print(f"✓ User: {user_info['metadata']['username']}")
        print(f"  Email: {user_info['metadata']['email']}")
        print(f"  Registered: {user_info['registered_at']:.0f}")
...
if __name__ == "__main__":
    ...
    # Run demos
    success = True
    success &= demo_basic_flow()
    success &= demo_device_binding()
    success &= demo_qr_flow()
    success &= demo_performance()
    success &= demo_revocation()
