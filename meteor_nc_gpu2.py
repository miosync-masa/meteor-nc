# meteor_nc_gpu2.py - APN (Adaptive Precision Noise) 実装
...
class MeteorNC_GPU:
    ...
    Example:
        >>> crypto = MeteorNC_GPU(n=256, m=10)  # APN enabled by default
        >>> crypto.key_gen()
        >>>
    ...
    def __init__(self,
                 ...
                 device_id: int = 0,
                 # ★ NEW: APN (Adaptive Precision Noise) - Default ON
    ...
    def encrypt_batch(self, messages: np.ndarray) -> np.ndarray:
        ...
        # ★ APN (Adaptive Precision Noise) - IND-CPA security
        if self.apn_enabled:
            magnitudes = cp.linalg.norm(C, axis=1, keepdims=True)  # [batch, 1]
    ...
    def verify_ind_cpa(self, num_samples: int = 100, verbose: bool = False) -> dict:
        ...
        Returns:
            dict: IND-CPA verification results
        """
        if len(self.public_keys_gpu) == 0:
            raise ValueError("Keys not generated. Call key_gen() first.")
...
def create_meteor_gpu(security_level: int = 256, 
                      device_id: int = 0,
                      apn_enabled: bool = True,
                      apn_safety_factor: float = 10000.0) -> MeteorNC_GPU:
    """
...
if __name__ == "__main__":
    print("="*70)
    print("Meteor-NC: GPU Implementation with APN (Adaptive Precision Noise)")
    print("="*70)

    # Create instance with APN enabled (default)
    ...
    # Verify Λ-security
    print("\n[*] Verifying Λ-security...")
    crypto.verify_security(verbose=True)

    # Verify IND-CPA
    print("\n[*] Verifying IND-CPA security...")
    crypto.verify_ind_cpa(verbose=True)

    # Benchmark
    print("\n[*] Running benchmark...")
    crypto.benchmark(verbose=True)

    # Cleanup
    crypto.cleanup()

    print("\n" + "="*70)
    print("✅ Meteor-NC with APN: IND-CPA secure & optimized!")
    print("="*70)
