...
Usage:
    from meteor_nc_string import MeteorNC_Practical
    
    # Initialization
    crypto = MeteorNC_Practical(n=256, m=10)
    crypto.key_gen()
    crypto.expand_keys()
...
# Note: meteor_nc_kdf.py is required at runtime
# from meteor_nc_kdf import MeteorNC_KDF, create_kdf_meteor

# =============================================================================
# Standalone implementation (testable without external dependencies)
# =============================================================================

class MeteorNC_Practical:
    """
    ...
    def decrypt_string(self, encrypted: dict) -> str:
        ...
        Args:
            encrypted: result of encrypt_string()
    ...
    def decrypt_bytes(self, encrypted: dict) -> bytes:
        ...
        Args:
            encrypted: result of encrypt_bytes()
    ...
    def deserialize_encrypted(self, json_str: str) -> dict:
        ...
        Args:
            json_str: output of serialize_encrypted()
            
        Returns:
            dict: ready for decryption
        """
...
def test_string_encryption():
    """String encryption test (mock version)"""
    print("=" * 70)
    print("Meteor-NC String Encryption Test (Mock)")
    print("=" * 70)
    ...
    for text in test_strings:
        ...
        vectors = crypto._bytes_to_vectors(data)
        recovered = crypto._vectors_to_bytes(vectors, len(data))
        
        match = data == recovered
        print(f"  '{text[:20]}...' ({len(data)} bytes) → "
              f"{vectors.shape} → {'OK' if match else 'FAIL'}")
        
        if not match:
            print(f"    Expected: {data[:50]}")
            print(f"    Actual:   {recovered[:50]}")
    
    print("\n[✓] Conversion logic test completed")
    print("=" * 70)
    print("Note: Full testing requires meteor_nc_kdf.py and a GPU")
    print("=" * 70)
...
# ==================================================================
# 
# Q: Why use 1 byte = 1 float64?
# A: For precision. Meteor-NC’s decryption error is ~1e-10,
#    so rounding perfectly restores the original byte.
#    Packing 8 bytes into one float64 accumulates errors
#    and risks bit flips.
# 
# Q: What is the overhead?
# A: 1:8 (byte : float64 byte size) + encryption overhead.
#    If compression is desired, apply gzip beforehand.
# 
# Q: Large files?
# A: Handled via chunk splitting. No issue as long as it fits in memory.
#    Streaming encryption is planned as a future enhancement.
# 
# ==================================================================
